...
class MYPDF extends TCPDF
{
  ...
  public function ColoredTable($header, $result)
  {
    ...
    $w = array(12, 35, 10, 25, 15, 27, 12, 20, 22,30,30,25);
    ...
    foreach ($result as $odt) {
      ...
      $this->Cell($w[1], 7, $odt['teach_name'], '1', 0, 'L', $fill);
      $this->Cell($w[2], 7, $odt['sex'], '1', 0, 'C', $fill);
      $this->Cell($w[3], 7, date("d/m/Y", strtotime($odt['birthday'])), '1', 0, 'C', $fill);
      $this->Cell($w[4], 7, $odt['status'], '1', 0, 'C', $fill);
      $this->Cell($w[5], 7, $odt['mobile'], '1', 0, 'C', $fill);
      $this->Cell($w[6], 7, $odt['ethic'], '1', 0, 'C', $fill);
      $this->Cell($w[7], 7, $odt['village'], '1', 0, 'C', $fill);
      $this->Cell($w[8], 7, $odt['dis_name'], '1', 0, 'C', $fill);
      $this->Cell($w[9], 7, $odt['pro_name'], '1', 0, 'C', $fill);
      $this->Cell($w[10], 7, $odt['subject'], '1', 0, 'C', $fill);
      $this->Cell($w[11], 7, $odt['subject_level'], '1', 0, 'C', $fill);
      // $this->Cell($w[12], 7, $odt['position'], '1', 0, 'C', $fill);
...
$pdf->setTitle('Report teacher');
$pdf->setSubject('Report teacher');
$pdf->setKeywords('Report teacher');
...
$pdf->writeHTMLCell(0, 0, '', 20, '<h2><u>ລາຍຊື່ຄູ-ອາຈານ ຂອງ ໂຮງຮຽນ ປະຖົມໂນນສະອາດ</u></h2>', 0, 0, 0, true, 'C', true);
...
// column titles
$pdf->Ln(2);
$header = array('#', 'ຊື່ ແລະ ນາມສະກຸນ', 'ເພດ', 'ວັນເດືອນປີເກີດ', 'ສະຖານະ', 'ເບີໂທ', 'ຊົນເຜົ່າ', 'ບ້ານ', 'ເມືອງ', 'ແຂວງ', 'ວີຊາສະເພາະ', 'ລະດັບການສຶກສາ');
