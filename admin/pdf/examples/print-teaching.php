...
$result = $api->getViewTeachingSubject();
...
class MYPDF extends TCPDF {
    ...
    public function ColoredTable($header,$result) {
        ...
        foreach ($result as $odt) {
            ...
            $this->Cell($w[2], 7, $odt['sub_name'], '1', 0, 'C', $fill);
...
$pdf->setTitle('Report Subject');
$pdf->setSubject('Report Subject');
$pdf->setKeywords('Report Subject');
...
$pdf->writeHTMLCell(0, 0, '', 20, '<h2><u>ລາຍວີຊາ</u></h2>', 0, 0, 0, true, 'C', true);
foreach ($result as $row) {
$pdf->writeHTMLCell(0, 0, '', 30, '<h4>ຫ້ອງ : '.$row['class_name'].' </h4>', 0, 0, 0, true, 'L', true);
$pdf->writeHTMLCell(0, 0, '', 30, '<h4>ສົກຮຽນ : '.$row['sch_name'].' </h4>', 0, 0, 0, true, 'R', true);
}
