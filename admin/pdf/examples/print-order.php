...
class MYPDF extends TCPDF {
    ...
    // Colored table
    public function ColoredTable($header,$order_detail) {
        ...
        foreach ($order_detail as $odt) {
            ...
            $this->Cell($w[3], 4, $odt['product_price'], '1', 0, 'C', $fill);
            $this->Cell($w[4], 4, $odt['qty'], '1', 0, 'C', $fill);
            $this->Cell($w[5], 4, $odt['price_qty'], '1', 0, 'C', $fill);
...
$pdf->writeHTMLCell(0, 0, '', 35, '<h2><u>ສັ່ງຊື້</u></h2>', 0, 0, 0, true, 'C', true);
$pdf->writeHTMLCell(0, 0, '', 39, '<h4>ເລກທີສັ່ງຊື້ : '.$order_no.' </h4>', 0, 0, 0, true, 'L', true);
$pdf->writeHTMLCell(0, 0, '', 42, '<h4>ວັນທີ: '.$date.'</h4>', 0, 0, 0, true, 'L', true);
...
// column titles
$pdf->Ln(2);
$header = array('#', 'ສິນຄ້າ', 'ປະເພດ', 'ລາຄາ', 'ຈ/ນ', 'ຈຳນວນເງິນ');


// print colored table
$pdf->ColoredTable($header, $order_detail);
