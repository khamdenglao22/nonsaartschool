...
$result = $api->getPrintScoreStudent();
...
class MYPDF extends TCPDF
{
    ...
    public function ColoredTable($header, $result)
    {
        ...
        foreach ($result as $odt) {
            ...
            $this->Cell($w[1], 7, $odt['sub_name'], '1', 0, 'L', $fill);
        ...
        $this->SetFont('phetsarathot', 'B', 12);
        $this->Ln(10);
        $this->Cell($w[0], 7, '', 0, 0, 'L', 0, '');
        $this->Cell($w[1], 7, 'ລາຍເຊັນອຳນວຍການ', 0, 0, 'L', 0, '');
        $this->Cell($w[2], 7, 'ລາຍເຊັນວີຊາການ', 0, 0, 'L', 0, '');
    }
}
...
if ($row['month'] == 13) {
    $pdf->writeHTMLCell(0, 0, '', 25, '<h2><u>ຄະແນນພາກຮຽນ I</u></h2>', 0, 0, 0, true, 'C', true);
} else {
    $pdf->writeHTMLCell(0, 0, '', 25, '<h2><u>ຄະແນນພາກຮຽນ II</u></h2>', 0, 0, 0, true, 'C', true);
}
$pdf->writeHTMLCell(0, 0, '', 35, '<h4>ຊື່ ແລະ ນາມສະກຸນ: ' . $row['st_name'] . ' </h4>', 0, 0, 0, true, 'L', true);
